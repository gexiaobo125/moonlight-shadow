...
void InitContact(struct Contact* ps) {
	memset(ps->data, 0, sizeof(ps->data));  //设置个人信息数组（data）的大小，并规定其初始值为0
	ps->size = 0;  //设置size初始值为0
}


void AddContact(struct Contact *ps) {
	if (ps->size == MAX) {        //当size=MAX时 即有MAX个个人信息 通讯录已经添加满
		printf("通讯录满了！\n");
	}
	else {
		printf("请输入名字：");
